    ...
    <>
      <div className="sticky top-0 z-50 bg-white w-full h-fit shadow-sm">
        <div className="flex w-full h-full justify-between gap-2 items-center px-2 md:px-6 py-2">
          ...
          <div className="flex gap-2 md:gap-4 font-medium text-sm items-center">
            <button
              className="rounded-full hidden lg:block hover:bg-slate-100 px-4 py-2 transition-all duration-300"
              onClick={() => {
                navigate(`/find-event/${located}`);
              }}
            >
            ...
            {isLoggedIn?.role?.toLowerCase() === "promotor" ? (
            ...
            {!isLoggedIn?.token && !isLoggedIn?.email ? (
              <>
                {" "}
                <button
                  className="rounded-full hover:bg-slate-100 px-4 py-2 transition-all duration-300"
                  onClick={() => {
                    navigate(`/login`);
                  }}
                >
      ...
      >
        <div className="flex flex-col">
          <div
            className="flex items-center gap-4 px-4 py-5 hover:bg-gray-100 cursor-pointer"
            onClick={() => {
              navigate(`/find-event/${located}`);
            }}
          >
          ...
          {isLoggedIn?.role?.toLowerCase() === "promotor" ? (
            <>
              ...
              <div className="flex items-center gap-4 px-4 py-5 hover:bg-gray-100 cursor-pointer">
                <div className="icon w-4 h-4">
                  <HiMiniCalendarDays size={"100%"} onClick={navigate("/promotor/create-event")} />
                </div>
