...
const ManageEvent = () => {
  ...
  const onHandleDelete = async (id) => {
    try {
      ...
      const deleteEvent = await API_URL_PROMOTOR.delete(`/manage-event/${id}`, {
  ...
  const onHandleEdit = async (id) => {
    try {
      const getToken = localStorage.getItem("token");
      const editEvent = await API_URL_PROMOTOR.patch(`/manage-event/${id}`, {
        headers: { Authorization: `Bearer ${getToken}` },
      });
    } catch (error) {}
  };
  ...
  const getDataEvent = async () => {
    ...
    try {
      ...
      const getEvent = await API_URL_PROMOTOR.get("/manage-event", {
        headers: { Authorization: `Bearer ${getToken}` },
      });
    ...
    <LayoutPromotor accountName={promotorName}>
      <div className="flex flex-col  mt-5 pb-20 md:ml-28">
        ...
        <ul>
          ...
          <div className=" pt-4 px-4 md:flex md:flex-col md:ml-[200px] lg:ml-[270px] mb-4 md:mb-[130px]"></div>
        </ul>
      </div>
      <Footer />
    </LayoutPromotor>
