...
const CreateEventProfile = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post("http://localhost:2066/accountDetail/registrasi", values, {
        headers: { Authorization: `Bearer ${JSON.parse(localStorage.getItem("token"))}` },
      });

      if (response.status === 201) {
        dispatch(register({ ...values, role: "EVENT ORGANIZER" }));
        localStorage.setItem("promotor", JSON.stringify({ ...values, role: "EVENT ORGANIZER" }));
  ...
  return (
    <div className=" w-full h-screen flex flex-col sm:flex-row items-start">
      <div className="w-full sm:w-1/2 h-1/2 sm:h-full bg-[#f5f5f5] flex flex-col p-4 sm:p-20 justify-between ">
        <h1 className=" text-2xl text-[#060606] font-semibold mt-4">Complete the form below</h1>
