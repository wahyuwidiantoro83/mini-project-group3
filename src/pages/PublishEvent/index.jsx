...
const PublishPage = () => {
  ...
  const onHandleClick = async () => {
    try {
      ...
      const createEvent = await API_URL_PROMOTOR.post(
        "/publish",
        { event: eventInfo, ticket: ticketInfo, promo: promoInfo },
  ...
  return (
    <LayoutPromotor>
        ...
        <div className=" pt-4 px-4 md:flex md:flex-col md:ml-[200px] lg:ml-[270px] mb-4 md:mb-[130px]">
          ...
          <div className="shadow-lg border-[1px] mt-4 md:flex">
            ...
            <div className="pl-3 pr-3 pt-3 pb-3 md:flex md:flex-col md:justify-between">
              ...
              <div>
                <p className="text-2xl font-bold">{eventBasic_infoParsed.eventTitle}</p>
                <p className="my-3">
                  {dayName[eventStart.getDay()]}, {eventStart.getDate()}{" "}
                  {monthName[eventStart.getMonth()]} {eventStart.getFullYear()}, at{" "}
                  {eventBasic_infoParsed.eventStartHour} - {eventBasic_infoParsed.eventEndHour}
                  <br />
                  {eventBasic_infoParsed.eventAddress}, {eventBasic_infoParsed.eventCity}{" "}
                </p>
              </div>
      ...
      <div className={`${openModalPublish ? "visible " : "invisible"}`}>{modalPublishBox()}</div>
      <Footer />
    </LayoutPromotor>
  );
};
